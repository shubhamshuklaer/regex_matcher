...
class dfa2mindfa:
	...
	def simplify(self, nfa, obj):
		nfa2dfaObj = obj
		states = list(nfa2dfaObj.dfa.states)
		n = len(states)
		unchecked = dict()
		count = 1
		distinguished = []
		...
		for i in range(n-1):
			for j in range(i+1, n):
				if not ([states[i], states[j]] in distinguished or [states[j], states[i]] in distinguished):
					eq = 1
					toappend = []
					for char in nfa2dfaObj.dfa.language:
						s1 = nfa2dfaObj.dfa.gettransitions(states[i], char)
						s2 = nfa2dfaObj.dfa.gettransitions(states[j], char)
						if len(s1) != len(s2):
							eq = 0
							break
						if len(s1) > 1:
		...
		if len(equivalent) == len(states):
			nfa2dfaObj.minDFA = nfa2dfaObj.dfa
		else:
			nfa2dfaObj.minDFA = nfa2dfaObj.dfa.newBuildFromEquivalentStates(equivalent, pos)
