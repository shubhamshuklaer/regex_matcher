...
class regex2nfa:
	...
	def buildNFA(self):
		...
		for char in self.regex:
			if char in self.alphabet:
				...
				self.automata.append(construct.basicstruct(char))
	...
	def processOperator(self, operator):
		...
		if operator == self.star:
			a = self.automata.pop()
			self.automata.append(construct.starstruct(a))
			...
			if operator == self.plus:
				self.automata.append(construct.plusstruct(b,a))
			elif operator == self.dot:
				self.automata.append(construct.dotstruct(b,a))
